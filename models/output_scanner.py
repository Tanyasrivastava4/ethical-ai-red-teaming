...
try:
    ...
    print("Warning: Detoxify init failed:", e)
    detox = None
...
try:
    ...
    print("Warning: HF toxic model load failed:", e)
    tokenizer = None
    hf_model = None

DETOX_THRESH = CFG["safety"].get("detox_threshold", 0.5)
HF_TOX_THRESH = CFG["safety"].get("hf_toxic_threshold", 0.55)
...
def scan_with_hf(text: str):
    if hf_model is None or tokenizer is None or not text:
...
def output_scan(text: str):
    ...
    pii = detect_pii(text)
    flagged = detox_res.get("flagged", False) or hf_res.get("flagged", False) or pii
    meta = {"detox": detox_res, "hf": hf_res, "pii": pii}
